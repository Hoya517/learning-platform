...
```
GET /api/chapters/{chapterId}/problems/random?userId=1&excludeProblemId=5
```
...
```java
AnswerStatus status = problem.gradeChoice(command.choiceNumbers());        // 객관식
AnswerStatus status = problem.gradeSubjective(command.subjectiveAnswer()); // 주관식
```
...
| Method | URL | 설명 |
|--------|-----|------|
| GET | `/api/chapters/{chapterId}/problems/random?userId=&excludeProblemId=` | 랜덤 문제 조회 |
